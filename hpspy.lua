--[[
  JOSEPEDOV V59 — MIDNIGHT CHASERS
...
subLbl.Text   = "JOSEPEDOV V59  ·  DELTA SAFE NO GC"
...
do
    ...
    -- Start RequestStreamAroundAsync NOW, in parallel with the
    -- ContentProvider:PreloadAsync download below.  Both run during
    -- the loading screen so neither blocks the other.
    local STREAM_POSITIONS = {
        Vector3.new(2464,  5,  583),  Vector3.new(2575,  5,  469),
        Vector3.new(2663,  5,  389),  Vector3.new(2771,  5,  501),
        Vector3.new(2852,  5,  566),  Vector3.new(2900,  5,  700),
        Vector3.new(2975,  5,  674),  Vector3.new(3087,  5,  636),
        Vector3.new(3190,  5,  637),  Vector3.new(3260,  12, 1016),
        Vector3.new(3276,  5,  847),  Vector3.new(3326,  5,  988),
        Vector3.new(3414,  5,  742),  Vector3.new(3474,  5,  715),
        Vector3.new(3485,  5,  622),  Vector3.new(3599,  5,  648),
    }
    local _streamingDone = false
    local _streamingTask = task.spawn(function()
        for _, spos in ipairs(STREAM_POSITIONS) do
            pcall(function() player:RequestStreamAroundAsync(spos, 10) end)
        end
        _streamingDone = true
    end)
    ...
    -- Wait for background streaming task to finish (started in
    -- parallel with PreloadAsync above).  On fast connections it
    -- will already be done.  Timeout 30 s to never hang.
    if _streamingTask then
        local deadline = tick() + 30
        while not _streamingDone and tick() < deadline do
            barTxt.Text = string.format("  55%%  —  Finishing map stream...")
            task.wait(0.1)
        end
    end
...
-- ── TRAFFIC SUPPRESSION (Delta-safe, no getconnections) ─────────────────
-- getconnections is a CrossExperience stub in Delta: calling it throws
-- uncatchable C-level errors that escape pcall and accumulate until the
-- script thread is terminated.  We suppress traffic purely with a
-- ChildAdded watcher that destroys NPC vehicles as they spawn — no
-- exploit API needed.
local _trafficConns = {}
...
local function ToggleTraffic()
    ...
    if Config.TrafficBlocked then
        -- Clear existing NPC vehicles immediately.
        for _,n in ipairs({"NPCVehicles","Traffic","Vehicles"}) do
            local f = Workspace:FindFirstChild(n)
            if f then
                pcall(function() f:ClearAllChildren() end)
                -- Watch the folder and destroy any vehicle that spawns.
                local conn = f.ChildAdded:Connect(function(child)
                    if Config.TrafficBlocked then
                        pcall(function() child:Destroy() end)
                    end
                end)
                table.insert(_trafficConns, conn)
            end
        end
    else
        -- Disconnect all watchers.
        for _,c in ipairs(_trafficConns) do
...
local function DoRaceLoop(uuidFolder)
    ...
    while Config.AutoRace and AR_STATE == "RACING" do
        ...
        repeat
            ...
            gatePart, cpIdx = FindNextCP(uuidFolder, clearedSet, nil)
        ...
        -- ── CHECKPOINT CLEAR ─────────────────────────────────────────
        -- Teleport the car's root part to the gate's exact position.
        -- Because we are the network owner of the car (seated in it),
        -- the server replicates our CFrame change and fires the gate's
        -- server-side Touched handler on the overlap.
        --
        -- firetouchinterest is NOT used — on Delta (and other mobile
        -- executors) it throws uncatchable C-level CrossExperience errors
        -- that escape pcall and terminate the script after ~8 calls.
        --
        -- MAX_ATTEMPTS then force-clear so we never loop forever.
        ...
        if not gatePart or not gatePart.Parent then
            -- Gate already gone or destroyed — count as cleared.
            cpCleared = true
        else
            ...
            if root then
                -- Watch for gate removal = server confirmed the touch.
                local cpConn   = nil
                ...
                -- CFrame at gate centre — car will physically overlap it.
                local teleportCF = CFrame.new(gatePart.Position)

                for attempt = 1, MAX_ATTEMPTS do
                    if cpCleared then break end
                    if not gatePart or not gatePart.Parent then cpCleared = true; break end
                    if not Config.AutoRace or AR_STATE ~= "RACING" then break end

                    SetStatus(string.format(
                        "🚀 CP #%s  [%d/%d]",
                        tostring(cpIdx), attempt, MAX_ATTEMPTS), 0, 190, 255)

                    -- Teleport car inside gate; server detects physics overlap.
                    pcall(function() car:PivotTo(teleportCF) end)
                    root.AssemblyLinearVelocity  = Vector3.zero
                    root.AssemblyAngularVelocity = Vector3.zero

                    -- Wait for server to replicate + fire Touched (up to 0.5s).
                    local deadline = tick() + 0.5
                    repeat
                        task.wait(0.05)
                    until cpCleared
                        or not gatePart.Parent
                        or tick() > deadline
                        or not Config.AutoRace
                        or AR_STATE ~= "RACING"

                    if not gatePart.Parent then cpCleared = true end
                end

                if cpConn then pcall(function() cpConn:Disconnect() end) end

                -- Force-clear after all attempts — never block progress.
                if not cpCleared then
                    SetStatus(string.format(
                        "⚠ CP #%s force-cleared", tostring(cpIdx)), 255, 180, 0)
                    cpCleared = true
                end

                -- Update coast direction for when no gate is visible.
                pcall(function()
                    if gatePart and gatePart.Parent then
                        lastDirXZ = Vector3.new(
                            gatePart.CFrame.LookVector.X,
                            0,
                            gatePart.CFrame.LookVector.Z)
                    end
                end)
            end
        end
        ...
        if cpCleared then
            ...
            SetStatus(string.format(
                "✓ CP #%s cleared", tostring(cpIdx)), 0, 230, 100)
            task.wait(0.05)
        end
        end
    end
...
TitleLbl.Text   = "🏁  MIDNIGHT CHASERS  V59"
...
print("\n═════════════════════════════════════════════════")
print("[J59] Midnight Chasers — V59 Delta Safe no getconnections")
print("[J59] Developed by josepedov")
print("[J59] Active Hooks: AutoRace, AutoFarm, MotoBoost, NoCrashDeath, Anti-AFK, Preloader+Streaming")
print("═════════════════════════════════════════════════\n")
